...
- `frontend/builder/diagram.controller.js` and `frontend/builder/ai.controller.js`: diagram studio and AI approval UI.
...
Required backend configuration: `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `DATABASE_URL`, `JWT_SECRET`; `ALLOWED_ORIGINS` is recommended in deployment. Optional AI configuration is `AI_API_URL`, `AI_API_KEY`, and `AI_MODEL`; the API key must remain server-side. The frontend and API are served together by `server/index.js` using `npm start` or `npm run dev`.
...
Diagram source is optional workflow data and supports `mermaid` or `markdown`. Markdown preview is sanitized. AI responses are treated as untrusted suggestions: only validated operations and explicit user approval may mutate the builder state.
